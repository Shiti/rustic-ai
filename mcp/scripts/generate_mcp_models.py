...
def generate_pydantic_models(
    ...
    tools_dict = [_tool_to_dict(tool) for tool in tools]

    # Create combined schema with all tool models
    combined_models = _create_combined_schema(tools_dict, provider, output_dir)
...
def _build_bound_mcp_config_comment(
    ...
    config_json = json.dumps({"server": server, "tools": tool_decls}, indent=2)
